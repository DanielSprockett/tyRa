...
`tyRa` ("tune-up your R analysis") is an R package that I created to facilitate building models from 16S rRNA amplicon data stored in `phyloseq` objects. 
...
spp.out <- fit_sncm(spp = otu_table(ps)@.Data, pool=NULL, taxon=data.frame(tax_table(ps)))
...
```{r sncm plot, message=FALSE, warning=FALSE}

plot_sncm_fit(spp.out, fill = NULL, title = "Test Plot")

```

As you can see from this plot, around 80% of the ASVs in this dataset fit the neutral model. 
